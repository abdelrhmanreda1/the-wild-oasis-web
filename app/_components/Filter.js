...
function Filter() {
  ...
  return (
    <div className="border border-primary-800 rounded-sm flex ">
...
function Button({ filter, activeFilter, children, handleFilter }) {
  return (
    <button className={`px-3  md:px-5 text-sm md:text-base py-2 m-1 rounded-md hover:bg-primary-700  ${filter === activeFilter ? "bg-primary-700 text-primary-50" : ""}`} onClick={() => handleFilter(filter)}>
      {children}
    </button>
  );
}
